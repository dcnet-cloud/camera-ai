...
- [`db.py`](fall_detection_web/db.py) — SQLite (WAL mode) for `events`, `recordings` (events are the recordings table too — filtered by presence of video fields), `users`, and `settings`. Schema is created/migrated in-code via `init_db` + `_ensure_column` (no migration framework). Old events/images auto-pruned (7-day retention). Event images on disk in `data/event_images/`.
