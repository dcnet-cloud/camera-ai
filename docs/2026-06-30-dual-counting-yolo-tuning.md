...
> **Working notes của session 2026-06-30.** Ghi lại: thay đổi UI trang chi tiết camera, hành trình tuning engine đếm YOLO, **kết quả thí nghiệm thực đo** (63 ảnh snapshot), menu giải pháp tầng-ngoài, config live hiện tại, và việc còn lại. Liên quan: tính năng dual-counting test (PR #8) — đã merge `main`.
...
## 11. PROD live + debug Axis-thumb / YOLO-data / clean-feed (2026-07-01)

Sau cutover, prod (`camera-test.dcnet.vn`, cam "DCNET - Lầu 2" cam_uid B8A44F4627CE) 2 bug + 1 cải tiến — đã fix, **cả 2 bộ đếm chạy thật**.

### 11.1 Axis không có thumbnail (fixed — PR #12)
Root cause: (1) cam seed `go2rtc_src` rỗng → `event_collector._save_axis_snapshot` thoát sớm; (2) `event_collector` KHÔNG mount `fdw_data` + thiếu `COUNTING_SNAPS_DIR` → ảnh không vào volume FDW serve. Fix git: `db.py` seed `go2rtc_src='cam_door'` + `rtsp_url='rtsp://go2rtc:8554/cam_door'`; compose collector thêm `fdw_data` volume + `COUNTING_SNAPS_DIR` + `GO2RTC_INTERNAL_URL`. Prod (row đã tồn tại, seed không re-run): UPDATE tay. Verified: 234 file snap, event mới `has_snap=t`, thumb serve public 200.

### 11.2 YOLO không có data (fixed)
Root cause: (1) `yolo_counting={}` → engine không start; (2) seed `rtsp_url=192.168.100.47` (IP dev) unreachable từ VM. Fix: rtsp_url→`rtsp://go2rtc:8554/cam_door` (go2rtc proxy cam, reachable), bật `yolo_counting`. Verified: `counter_yolo` in/out có data thật.

### 11.3 Vạch đếm khớp choke cửa
Ban đầu y50 x[38,72] (đuôi trái nhiễu). User vẽ vạch vàng = ngưỡng sàn trước 2 cửa kính. Chốt **line_y=52, x[47,68]** (khớp vàng, bỏ đuôi trái) — "chắc chắn ra/vào phải bước qua". Model n@640 conf 0.3. Config runtime trong `cameras.yolo_counting` (KHÔNG trong git).

### 11.4 Clean feed — tắt overlay burn-in cam Axis (VAPIX)
YOLO ăn frame RTSP **đã có overlay Axis burn sẵn** (khung đỏ + icon ↓↑ đếm + trajectory) — cam tự vẽ, KHÔNG phải YOLO. Detect vẫn OK (conf 0.85) nhưng không sạch. Fix: VAPIX `param.cgi?action=update&Image.I0.Appearance.Overlays=off` (value hợp lệ: all/all-sync/**off**/text/image/application; `none` INVALID). ⚠️ **Phải restart go2rtc** (mở session RTSP mới) mới ăn — frame cache session cũ vẫn overlay. ⚠️ **KHÔNG ảnh hưởng đếm Axis MQTT** (AOA scenario vẫn active, chỉ tắt vẽ). Rollback: set lại `all`. Cam = AXIS M3216-LVE, root VAPIX qua `https://<CAM_IP>:8443` **`--anyauth`** (digest fail, basic OK). Nguyên tắc chung: cam đưa qua YOLO tầng-ngoài nên dùng **feed gốc** (cam không native-detect thì vốn raw).

### 11.5 Trạng thái prod (2026-07-01)
- 4 service up; mosquitto+caddy giữ. Cả `counter` (Axis) + `counter_yolo` có data thật + thumbnail. Feed sạch (overlay off).
- MQTT client-id prod: `event_collector_cameraai_prod_<rand>` (tránh instance lạ `171.243.48.224` đá session).
