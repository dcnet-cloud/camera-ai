...
- Tab Live để xem live bằng go2rtc stream hoặc snapshot refresh.
...
- `Video`: xem go2rtc stream nếu có `go2rtc_url`; nếu không thì xem snapshot refresh.
...
Ví dụ automation với MQTT `frigate/events`:

```yaml
automation:
  - alias: "Simple AI Vision - Frigate person bep"
    trigger:
      - platform: mqtt
