...
* **Database & Auth:** PostgreSQL (psycopg v3, psycopg-pool) — bảng `incidents` (cũ: `events`), `users`, `settings`; bcrypt, python-jose. DSN qua env `DATABASE_URL`. (Phase 0: đã migrate từ SQLite sang Postgres — xem `docs/specs/2026-06-26-dcnet-platform-migration-design.md`.)
